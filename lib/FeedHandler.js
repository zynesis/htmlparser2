...
function getDescription (obj, prop, what, where, recurse) {
	...
	if (!desc) {
		tmp = DomUtils.getInnerHTML(DomUtils.getElementsByTagName(what, where, recurse)[0]);
		if (tmp) {
			desc = removeCommentTags(tmp);
		}
	}
	...
	if (!content) {
		tmp = DomUtils.getInnerHTML(DomUtils.getElementsByTagName('content:encoded', where, recurse)[0]);
		if (tmp) {
			content = removeCommentTags(tmp);
		}
